...
void ConservativeNegativeLaplacianSameLevel128(HADeviceGrid<Tile>& grid, thrust::device_vector<HATileInfo<Tile>>& tiles, int launch_tile_num, int subtree_level, uint8_t launch_tile_types, int x_channel, int Ax_channel, bool calc_diag = false);
...
void ConservativeFullNegativeLaplacian(HADeviceGrid<Tile>& grid, const int x_channel, const int Ax_channel, bool calc_diag = false);
...
public:
    ...
    CMGSolver(T _one_over_alpha = 1, T _prolong_coeff = 1) : d_tmp(7), one_over_alpha(_one_over_alpha), prolong_coeff(_prolong_coeff) {
