...
void FluidEuler::mixFluidAndSolidVelocityOnFaces(HADeviceGrid<Tile>& grid, const double current_time, const double dt, const int coeff_channel, const int u_fluid_channel, const int u_mix_channel) {
	...
	grid.launchVoxelFuncOnAllTiles(
		...
		for (int axis : {0, 1, 2}) {
			...
			if (solid_ratio > 0) {
				mix_vel += solid_ratio * FluidScene::solidFaceCenterVelocity(params, current_time, dt, acc, info, l_ijk, axis);
			}
...
{
	auto params = mParams;
	auto xform = FluidScene::meshToWorldTransform(params, current_time);
...
void FluidEuler::init(json &j, DriverMetaData &metadata) {
...

	{
		...
		grid.iterativeRefine([=]__device__(const HATileAccessor<Tile> &acc, HATileInfo<Tile> &info) ->int {
			return FluidScene::initialLevelTarget(params, acc, info);
		});
	}
...
void FluidEuler::buildTypesAndAMGCoeffsFromNodeSDFs(HADeviceGrid<Tile>& grid, const T current_time) {
	if (mMeshSDFAccel != nullptr) {
		auto params = mParams;
		grid.launchVoxelFuncOnAllTiles(
			[=] __device__(HATileAccessor<Tile>&acc, HATileInfo<Tile>&info, const Coord & l_ijk) {
			FluidScene::setWallCellType(params, current_time, acc, info, l_ijk);
		}, LEAF
		);
...
{
	if (params.mTestCase == MESHMOTION || params.mTestCase == AIRCRAFT || params.mTestCase == SPHERECIRCLING || params.mTestCase == JASSM) {
		bool is_neumann = false;
		bool is_dirichlet = false;
		int boundary_axis, boundary_off;

		if (QueryBoundaryDirectionN1P1OnCoarseLevel(acc, params.mCoarseLevel, level, g_ijk, boundary_axis, boundary_off)) {
			is_neumann = true;
		}
...
__hostdev__ uint8_t wallCellType(const FluidParams& params, const T current_time, const HATileAccessor<Tile>& acc, const HATileInfo<Tile>& info, const nanovdb::Coord& l_ijk) {
	auto g_ijk = acc.localToGlobalCoord(info, l_ijk);
	return wallCellType(params, current_time, acc, info.mLevel, g_ijk);
}
...
{
	auto& tile = info.tile();
	tile.type(l_ijk) = wallCellType(params, current_time, acc, info, l_ijk);
}

__hostdev__ Eigen::Transform<T, 3, Eigen::Affine> meshToWorldTransform(const FluidParams& params, const T current_time)
{
	if (params.mTestCase == MESHMOTION) {
		T t = current_time;
		double jitter = 2.9e-4;
		const T x = 0.5 + jitter;
		const T y = 0.5 + jitter;
		const T z0 = 0.3 + jitter;
		const T z1 = 0.3 + jitter;
	...
	else if (params.mTestCase == AIRCRAFT) {
	...
	if (params.mTestCase == SPHERECIRCLING) {
		T t = current_time;
		if (t < (T)0) t = (T)0;
		if (t > (T)2) t = (T)2;
	...
	else if (params.mTestCase == JASSM) {
		using Vec3 = Eigen::Matrix<T, 3, 1>;
		...
		CUDA_ASSERT(false, "meshToWorldTransform not implemented for test case %d", int(params.mTestCase));
	}
}
...
{
	if (params.mTestCase == MESHMOTION || params.mTestCase == AIRCRAFT || params.mTestCase == SPHERECIRCLING || params.mTestCase == JASSM) {
		Vec wall_vel(0, 0, params.mesh_motion_inflow);
		auto g_ijk = acc.localToGlobalCoord(info, l_ijk);
		auto ng_ijk = g_ijk; ng_ijk[axis]--;
		if (wallCellType(params, current_time, acc, info.mLevel, g_ijk) == NEUMANN || wallCellType(params, current_time, acc, info.mLevel, ng_ijk) == NEUMANN) {
			return wall_vel[axis];
		}
		else {
			auto pos0 = acc.faceCenter(axis, info, l_ijk);
			auto T0 = meshToWorldTransform(params, current_time);
			auto T1 = meshToWorldTransform(params, current_time + dt);
